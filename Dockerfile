...
RUN apt-get update -qqy && \
        apt-get upgrade -qqy && \
        apt-get install -qqy tzdata wget curl tmux less vim cmake git software-properties-common build-essential nfs-common jq python3-pip  && \
        apt-get clean -qqy
...
RUN wget "https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2004/x86_64/cuda-ubuntu2004.pin" -O /etc/apt/preferences.d/cuda-repository-pin-600 \
        ...
        apt-get install -qqy --no-install-recommends nvidia-utils-495 libgl-dev cuda-11-5 libcudnn8 libcudnn8-dev && \
        apt-get clean -qqy
